  ...
  async handle(event: RecruitmentCreatedEvent) {
    await this.prisma.recruitmentProjectionUpdater.create({
      data: {
        ...
        destinationPrefecture: event.destination.latitude,
        destinationAddress: event.destination.longitude,
        destinationDescription: event.destination.description,
        depaturePrefecture: event.depature.latitude,
        depatureAddress: event.depature.longitude,
